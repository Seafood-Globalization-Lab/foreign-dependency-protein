...
aa_reliance_mean_global_trends <- aa_reliance_mean_global_trends %>%
...
aa_reliance_multiple_trends_alt <- aquatic_reliance %>%
  ...
  filter(habitat != "unknown") %>%
   filter(!(year %in% c(2010, 2011, 2012, 2013) & data_source == "old FBS")) # Remove overlapping years,
...
(aquatic_reliance_rank_change <- aquatic_reliance_ranks %>%
  ...
  mutate(country_name = countrycode(iso3c, origin = "iso3c", destination = "country.name"),
         country_name = case_when(country_name == "Micronesia (Federated States of)" ~ "Micronesia", TRUE ~ country_name),

...
(one <- fao_aquatic_source %>%
  ...
  scale_x_continuous(breaks = ~ axisTicks(., log = FALSE)) +
   lims(y = c(0,16))) # makes x axis transparent
...
```{r}
# Get all capture data that is greater than 2010 - determine roc in capture usage
aa_roc <- fao_aquatic_source %>%
  filter(year > 2010, data_source == "new FBS", Element == "Proportion animal source protein from aquatic") %>%
  group_by(iso3c, year) %>%
  summarize(value = sum(value)) %>%
  split(.$iso3c) %>% 
  map(~lm(value~year, data = .x)) %>% 
...
# estimate.x = capture fishery, estimate.y = aquaculture
(  <- left_join(terrestrial_roc,aa_roc, by = c("iso3c" = "consumer_iso3c")) %>%
  add_region(col = "iso3c", region.col.name = "region") %>%
...
ggsave("../images/fig_five_b.png", fig_five, device = "png", width = 6, height = 4, units = "in", bg = "transparent")
```

...
?fct_reorder(iso3c, .fun = prop_animal_protein)
# Add ranked bar plots
aquatic_reliance %>%
  filter(year == 2019,
         food_group == "aquatic",
         habitat == "inland",
  ...
  group_by(iso3c) %>%
  summarize(prop_animal_protein = 100 * sum(prop_animal_protein)) %>%
  mutate(iso3c = fct_reorder(iso3c, prop_animal_protein)) %>%
  top_n(10) %>%
  ggplot(aes(x = prop_animal_protein, y = iso3c)) +
  geom_col()
```
...
```{r}
  lm(aquatic_reliance_estimate ~ aquatic_estimate * region + terrestrial_estimate * region, data = plot_df) %>%
    summary()
  library(lme4)
  
model <- lmer(aquatic_reliance_estimate ~ aquatic_estimate + terrestrial_estimate +
       (aquatic_estimate + terrestrial_estimate | region),
     data = plot_df)

re <- ranef(model, condVar = TRUE)  # condVar=TRUE lets us extract uncertainty later
re_region <- re$region


df_re <- as_tibble(re_region, rownames = "region")
df_long <- df_re %>%
  pivot_longer(cols = c(aquatic_estimate, terrestrial_estimate),
               names_to = "slope_type",
               values_to = "slope_value")


ggplot(df_long,
       aes(x = region, y = slope_value, fill = slope_type, group = slope_type)) +
  geom_col() +
  theme_minimal() +
  labs(title = "Random Slopes by Region",
       x = "Region", y = "Random Slope Value",
       color = "Slope Type") +
...
```{r}
pre_df <- aquatic_reliance %>%
  group_by(year, iso3c, data_source, food_group) %>%
  summarize(protein_consumed_t = sum(protein_consumed_t)) %>%
  pivot_wider(values_from = "protein_consumed_t", names_from = "food_group") %>%
  mutate(aquatic_reliance = Aquatic / (Aquatic + Terrestrial)) %>%
  add_region(col = "iso3c", region.col.name = "region")

dataset <- {lmer(aquatic_reliance ~ Aquatic + Terrestrial +
       (Aquatic + Terrestrial | region),
     data = pre_df) %>%
  ranef()}$region
 ...
 dataset%>%
  ...
  summarize(`(Intercept)` = mean(`(Intercept)`),
           Aquatic = mean(Aquatic),
           Terrestrial = mean(Terrestrial)) %>%
  pivot_longer(cols = c("Aquatic", "Terrestrial")) %>%

...
aquatic_animal_proteins <- fao_food_long %>%
  filter(Item %in% c("Aquatic Animals, Others", "Cephalopods",
...
fao_prop_imports <- aquatic_animal_proteins %>% 
  ...
  group_by(iso3c, year) %>%
  summarize(prop_aquatic_animal = a_a_protein / (t_p + a_a_protein), 
            a_a_protein = sum(a_a_protein))
...
supply_importance <- artis_fao_fbb %>%
  mutate(aa_reliance = prop_consumption * prop_aquatic_animal, 

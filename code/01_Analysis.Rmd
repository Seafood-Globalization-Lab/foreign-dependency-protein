...
source("../utils/standardize_country_data.R")
```
...
# Since loading in ARTIS data takes awhile, only run if consumption data has not been obtained before
if (!file.exists("../data/consumption_full.parquet")) {
  
  # Set directory for ARTIS database folder
  db_folder <- "K:/data-storage/artis/ARTIS_2.0_FAO_2025_09_09"
  
  # Open ARTIS consumption
  artis_ds <- arrow::open_dataset(file.path(db_folder, "datasets", "ARTIS_v2.0_consumption_FAO_mid_all_HS_yrs_2025-09-12.parquet"))
  ...
  consumption_full <- artis_ts_result %>%
    ...
    mutate(
      consumption_source = str_replace(consumption_source, " step 1", ""),
  ...
  write_parquet(consumption_full, "../data/consumption_full.parquet")  
...
fao_food <- read_csv("../data/FAO_FBS/FoodBalanceSheets_E_All_Data/FoodBalanceSheets_E_All_Data_2026.csv")
...
fao_historical <- read_csv("../data/FAO_FBS/FoodBalanceSheetsHistoric_E_All_Data/FoodBalanceSheetsHistoric_E_All_Data.csv") %>%
...
fao_new_raw <- read_parquet("../output/fao_food.parquet") %>% 
  ...
  select(-ends_with("F"), 
         -ends_with("N")) %>%
  pivot_longer(cols = Y2010:Y2023, names_to = "year") %>%
  ...
  ungroup() %>%
  mutate(data_source = "new FBS") %>%
  # Convert to kg/cap/yr
  mutate(value = case_when(
    Element == "Protein supply quantity (g/capita/day)" ~ (value / 1000) * 365,
    TRUE ~ value
  ),
  Element = case_when(Element == "Protein supply quantity (g/capita/day)" ~ "Protein supply quantity (kg/capita/yr)",
                      TRUE ~ Element),
  Unit = case_when(Element == "Protein supply quantity (kg/capita/yr)" ~ "(kg/capita/yr)",
                   TRUE ~ Unit)
  )
...
# Calculate protein consumption in tonnage per year for old FBS
# To do this we need to aggregate g/capita/day across population values
...
fbs_old_pops <- read_parquet("../output/fao_historical.parquet") %>%
  ...
  # Only keep years 1996 and after to align with ARTIS
  filter(year >= 1996) %>%
  # Convert units from thousands of people to 1's of people. Right now the
  # United States is measured as 300,000 1000 people, but I am converting this
...
# Convert g/capita/dat to tonneage per year
protein_supply_t_old <- read_parquet("../output/fao_historical.parquet") %>%
    # Remove aggregated regions
    filter(`Area Code` < 1000, Area != "China") %>% # China included in other names that FBS has
    # Remove flag columns
    select(-ends_with("F")) %>%
    pivot_longer(cols = Y1961:Y2013, names_to = "year") %>%
    # Convert year column to numeric after removing "Y" prefix
  ...
  filter(Element == "Protein supply quantity (g/capita/day)") %>%
    mutate(
      value = ((value / 1e06) * population * 365),
...
# Add into old FBS measures
fao_old_raw <- bind_rows(fao_old_raw, protein_supply_t_old)
...
# Comparison: Population values from FAO/WB

To understand how population data differs (i.e., important for calculating per capita values), we compare the FAO-FBS population values 
...
# Summarize dfs that include aquatic animal consumption
...
elements_select <- c("Food supply (kcal)", 
                     "Protein supply quantity (kg/capita/yr)",
...
aquatic_animals <- c("Cephalopods",
                     ...
                     "Molluscs, Other", "Pelagic Fish", "Aquatic Animals, Others")
...
consumption_props <- consumption_full %>%
  ...
  mutate(prop_aquatic_source = consumption_live_t/sum(consumption_live_t)) %>%
  select(-consumption_live_t) %>%
  ungroup()

# Test for NAs
consumption_props %>%
  filter(is.na(prop_aquatic_source))

# Test that all add back to 1
test <- consumption_props %>%
  group_by(year, consumer_iso3c) %>%
  summarise(test_val = sum(prop_aquatic_source)) %>%
...
# Calaculate % protein from aquatic animals

```{r calculate percent of protein from aquatic animals}
# Calculate aggregate aquatic animal protein dominance from new FBS
fao_new_prop_aquatic <- fao_new_raw %>%
  ...
  mutate(
    aquatic_dominance = aquatic_animal_protein/total_animal_protein,
    terrestrial_dominance = (total_animal_protein - aquatic_animal_protein)/total_animal_protein
  ) %>%
  # Reshape to long format with food group indicator
  pivot_longer(
    cols = c(aquatic_dominance, terrestrial_dominance),
    names_to = "food_group_temp",
    values_to = "value"
  ) %>%
  mutate(
    food_group = case_when(
      food_group_temp == "aquatic_dominance" ~ "aquatic",
      food_group_temp == "terrestrial_dominance" ~ "terrestrial"
    ),
...
# Extract per capita protein consumption as it's own separate column to join
# back into dominance datasets

## New FBS
new_protein_consumed_kg_cap_yr <- fao_new_raw %>%
  # Filter to the total animal product protein supply
  ...
  # Get total protein in tonnage
  mutate(protein_consumed_kg_cap_yr = case_when(
    food_group == "aquatic" ~ aquatic_animal_protein,
    food_group == "terrestrial" ~ total_animal_protein - aquatic_animal_protein
  )) %>%
  select(year, iso3c, data_source, food_group, Element, Unit, protein_consumed_kg_cap_yr)

## Old FBS
old_protein_consumed_kg_cap_yr <- fao_old_raw %>%
  # Filter to the total animal product protein supply
  filter(Element == "Protein supply quantity (kg/capita/yr)",
         Item == "Animal Products") %>%
  select("year", "iso3c", "data_source", "total_animal_protein" = "value") %>%
...
# Join on protein consumption estimates to dominance datasets

## New FBS
fao_new_prop_aquatic <- fao_new_prop_aquatic %>%
  left_join(new_protein_consumed_kg_cap_yr,
            by = c("year", "iso3c", "data_source", "food_group", "Element", "Unit"))

## Old FBS
fao_old_prop_aquatic <- fao_old_prop_aquatic %>%
  left_join(old_protein_consumed_kg_cap_yr,
            by = c("year", "iso3c", "data_source", "food_group", "Element", "Unit"))

# Create joined data frames of aquatic food sourcing
fao_aquatic <- fao_combined_food_groups %>%
  left_join(joined_population, 
            by = c("year", "iso3c", "data_source")) # Get total protein in tonnage

# Combine old / new FBS for the aggregate aquatic dominance dataset
fao_aquatic_dominance <- fao_new_prop_aquatic %>%
  bind_rows(fao_old_prop_aquatic)

rm(list = c("fao_new_aquatic", "fao_old_aquatic", "fao_new_prop_aquatic", "fao_old_prop_aquatic"))
```

## Disaggregate values by production source

```{r disaggregate by source + create final output datasets}
#######################################
###     AQUATIC SOURCE DATASET      ###
#######################################

# Join FAO source data to ARTIS consumption sourcing proportions
fao_aquatic_source <- fao_aquatic %>%
  left_join(consumption_props %>%
              mutate(food_group = "aquatic"), 
            by = c("year", "iso3c" = "consumer_iso3c", "food_group"),
            # FIXIT: confirm this should be many-to-many
            relationship = "many-to-many") %>%
  # Disaggregate proportionately to ARTIS consumption sourcing
  mutate(value = value*prop_aquatic_source) %>%
  clean_names() %>%
  # Filter out na habitat and methods for aquatic foods
  filter((food_group == "aquatic" & !is.na(habitat)) | 
         (food_group == "aquatic" & !is.na(method)) | 
           food_group == "terrestrial")

#######################################
###    AQUATIC dominance DATASET     ###
#######################################

# Disaggregate FAO animal protein dominance data
fao_aquatic_dominance_source <- fao_aquatic_dominance %>%
  left_join(consumption_props %>%
              mutate(food_group = "aquatic"), 
            by = c("year", "iso3c" = "consumer_iso3c", "food_group"),
            # FIXIT: confirm this should be many-to-many
  ...
  mutate(
    prop_animal_protein = case_when(
      food_group == "aquatic" ~ value*prop_aquatic_source,
      TRUE ~ value
    ),
    protein_consumed_t = case_when(
      food_group == "aquatic" ~ protein_consumed_t*prop_aquatic_source,
      TRUE ~ protein_consumed_t
      ),
    protein_consumed_kg_cap_yr = case_when(
      food_group == "aquatic" ~ protein_consumed_kg_cap_yr*prop_aquatic_source,
      TRUE ~ protein_consumed_kg_cap_yr
    )
    ) %>%
  select(-value, -Element, -Unit) %>%
  clean_names() %>%
  # Filter out na habitat and methods for aquatic foods
  filter((food_group == "aquatic" & !is.na(habitat)) | 
         (food_group == "aquatic" & !is.na(method)) | 
           food_group == "terrestrial") %>%
  relocate(protein_consumed_kg_cap_yr, .before = prop_animal_protein) %>%
  relocate(protein_consumed_t, .before = protein_consumed_kg_cap_yr) %>%
  rename(protein_consumed_t_peryr = protein_consumed_t,
         protein_consumed_kg_percap_peryr = protein_consumed_kg_cap_yr)

#######################################
###         POPULATION DATA         ###
#######################################

# Create separate table for population data
...
test <- fao_aquatic_source %>% 
  ...
  summarise(test = sum(prop_aquatic_source)) %>%
...
a <- fao_aquatic_source %>%
  ...
  filter(is.na(value) | is.na(prop_aquatic_source) | is.na(iso3c))
...
# Confirm proportions add back to 1
(test <- fao_aquatic_dominance_source %>% 
  group_by(year, iso3c, data_source, food_group) %>%
  summarise(test = sum(prop_aquatic_source)) %>%
  filter(abs(test-1) > 0.0001))

# Confirm aquatic animal + terrestrial dominance for a given country's year adds to 1
test <- fao_aquatic_dominance_source %>%
...
# Result: Countries that do not add up to one are only countries that that include the terrestrial dominance
# i.e., these countries do not have ARTIS consumption data, so we do not have disaggregate aquatic animal protein
# dominance.
if(nrow(test) > 0){
  warning("Disaggregated aquatic food dominance proportions do not sum to 1")
}

# Check for NA values in aquatic dominance source dataset
a <- fao_aquatic_dominance_source %>%
  filter(food_group == "aquatic") %>%
  filter(is.na(prop_animal_protein) | is.na(prop_aquatic_source) | is.na(iso3c))

if(nrow(a) > 0) {
  warning("There is an NA")
}

# Look at attributes of aquatic food only from the dominance dataset
summary(fao_aquatic_dominance_source %>%
          filter(food_group == "aquatic"))
...
# Test that joining population data does not add any rows
nrow_start <- fao_aquatic_dominance_source %>%
  nrow()
nrow_end <- fao_aquatic_dominance_source %>%
  left_join(population_fao_wb, by = c("year", "iso3c")) %>%
  nrow()

# Result: Joining population data does not add any rows
paste0("Check: population data does not add any rows: ", nrow_start - nrow_end == 0)  # desired output: TRUE

# Look at attributes of joined dataset
summary(fao_aquatic_dominance_source %>%
          left_join(population_fao_wb, by = c("year", "iso3c")))

# Check global per capita time series
# From FAO SOFIA: " Globally, aquatic animal foods supplied 15 percent of  animal proteins and 6 percent of all proteins in 2021
# Global values
df <- fao_aquatic_dominance_source %>%
  group_by(year, data_source, food_group) %>%
  summarise(protein_consumed_kg_percap_peryr = sum(protein_consumed_t_peryr)) %>%
  group_by(year, data_source) %>%
  mutate(percent = 100*protein_consumed_kg_percap_peryr/sum(protein_consumed_kg_percap_peryr))

# Country values
df <- fao_aquatic_dominance_source %>%
  group_by(year, iso3c, data_source, food_group) %>%
  summarise(protein_consumed_kg_percap_peryr = sum(protein_consumed_kg_percap_peryr)) %>%
  group_by(year, iso3c, data_source) %>%
  mutate(percent = 100*protein_consumed_kg_percap_peryr/sum(protein_consumed_kg_percap_peryr))

# Clean up environment
keep <- c("fao_aquatic_source", "fao_aquatic_dominance_source", "population_fao_wb") # Keep only output files in env
rm(list = setdiff(ls(), keep))
gc(full = TRUE) # Clean up memory usage
```

# Export output files

```{r write files to .csv}
# Write aquatic dominance source file to csv
write.csv(fao_aquatic_source,
          "../output/fao_fbs_aquatic_source.csv", row.names = FALSE)
 
# # Write aquatic dominance data to csv
write.csv(fao_aquatic_dominance_source,
          "../output/fao_aquatic_dominance_source.csv", row.names = FALSE)

# # Write population data to csv
write.csv(population_fao_wb, "../output/population_fao_wb.csv", row.names = FALSE)
```

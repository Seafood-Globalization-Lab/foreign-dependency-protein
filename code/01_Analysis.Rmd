...
fao_old_raw <- read_parquet("../output/fao_historical.parquet") %>% 
  ...
  mutate(value = case_when( # Derive annual protein supply (we're going to recalculate per capita since FBS doesn't aggregate fully by country (but separates by territory)
    Element == "Protein supply quantity (g/capita/day)" ~ (value * population * 365) / 1e7,
    TRUE ~ value
  ),
...
fao_aquatic_reliance_source <- fao_aquatic_reliance  %>%
  ...
  filter((food_group == "aquatic" & !is.na(habitat)) | 
         (food_group == "aquatic" & !is.na(method)) | 
           food_group == "terrestrial") %>%
  relocate(protein_consumed_t, .before = "prop_animal_protein")
...
df <- fao_aquatic_reliance_source %>%
  ...
  summarise(protein_consumed_t = sum(protein_consumed_t)) %>%
  group_by(year, data_source) %>%
  mutate(percent = 100*protein_consumed_t/sum(protein_consumed_t))
